...
switch ($_REQUEST['action']) {
	...
	case 'sendmessage':
		...
		if(empty($_REQUEST['isGroup']) || $isGroup == 0) { 
			if(empty($message)) {
				$response['error'] = "You can not post empty Message.";
			} else {
				$type = '';   
			...
			if(empty($message)) {
				...
				if($isGroupJoined) {
					...
					$response['error'] = "You dont have Permission to Send  Message to this Group.";
				}		
			}		
