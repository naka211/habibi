...
(function($) {
    ...
    $.crembedded = (function() {
        return {
                showLoggedOutLayout: function(params){
                    if(crguestsMode == 0){
                        $('#cometchat').html('<div id="cometchat_loggedout_container"><div id="cometchat_loggedout"><div><img class="cometchat_loggedout_icon" src="'+staticCDNUrl+'layouts/'+settings.themename+'/images/exclamation.png" /></div><div>'+"<?php echo $chatrooms_language['loggedout_tooltip'];?>"+'</div></div></div>');
                        return;
                    }
                },
                        ...
                        if (message != '' && incoming.chatroomid == jqcc.cometchat.getChatroomVars('currentroom') && typeof(message) != 'undefined') {
                                ...
                                if($("#cometchat_groupmessage_"+incoming.id).length > 0) {
                                    ...
                                    if(!incomingself) {
                                        ...
                                        var defaultAvatarURL = staticCDNUrl + 'admin/images/pixel.png';
                                        var fromavatar = '<a id="cometchat_usersavatar_'+incoming.id+'" href="javascript:void(0)" onclick="javascript:parent.jqcc.cometchat.chatWith(\''+incoming.fromid+'\');" class="cometchat_float_l"><span class="cometchat_cr_other_avatar"><img class="cometchat_userscontentavatarsmall cometchat_avatar_'+incoming.fromid+'" title="'+fromname+'" src="'+avatar+'" onerror="this.onerror=null;this.src=\'' + defaultAvatarURL + '\'"></span></a>';
                                        var usernamecontent = '';
                                        if (mobileDevice || showUsername == '1') {
                                            usernamecontent = '<span class="cometchat_groupusername">'+fromname+':</span>';
                                        }
                    ...
                    updateChatroomUsers: function(item,fetchedUsers) {
                        ...
                        $.each(item, function(i,user) {
                            ...
                            if(parseInt(user.b)!=1) {
                                ...
                                if(user.a != '') {
                                    avatar = '<span class="cometchat_userscontentavatar"><img class="cometchat_userscontentavatarimage" src="'+user.a+'" onerror="this.onerror=null;this.src=\'' + defaultAvatarURL + '\'"></span>';
                                }
                                newUsers[user.id] = 1;
                        ...
                        $.each(data, function(i, incoming){
                            ...
                            if (message != '') {
                                ...
                                if (!incomingself) {
                                    ...
                                    var defaultAvatarURL = staticCDNUrl + 'admin/images/pixel.png';
                                    var fromavatar = '<a id="cometchat_usersavatar_'+messagewrapperid+'" href="javascript:void(0)" class="cometchat_float_l"><span class="cometchat_cr_other_avatar"><img class="cometchat_userscontentavatarsmall cometchat_avatar_'+incoming.fromid+'" title="'+fromname+'" src="'+avatar+'" onerror="this.onerror=null;this.src=\'' + defaultAvatarURL + '\'"></span></a>';
                                    var usernamecontent = '';
                                    if (mobileDevice || showUsername == '1') {
                                        usernamecontent = '<span class="cometchat_groupusername">'+fromname+':</span>';
                                    }
        ...
        if (cometchat_container.hasClass('visible')){
            ...
            $('#cometchat_righttab').find('.moderatorbutton').on('click',function(e){
                ...
                if(method == 'cc_kick' && confirm("Are you sure!")) {
                   ...
                   $('#cometchat_righttab').find("#chatroom_userlist_"+uid).remove();
